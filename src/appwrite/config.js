...
export class Service {
  ...
  async createSession(email, password) {
    try {
      return await this.account.createSession(email, password); // Create a session for login
    } catch (error) {
  ...
  async getAccount() {
    try {
      ...
      throw error; // Handle error if user is not authenticated
    }
  }

  async deleteSession() {
    try {
      return await this.account.deleteSession('current'); // Logout the user
    } catch (error) {
      throw error;
    }
  }
...
async createPost({title, slug, content, featuredImage, status, userId}) {
    try {
      ...
      console.log("Error creating post:", error);  // Log error for better debugging
      throw error;
    }
  }
